...
def plot_nodes(mypath, filename,b,data,t, parameters = None):
        ...
        for i in range(1,len(data)+1):
            ...
            if data[i-1,0]== b[n-1,]:
                ...
                if int(data[i-1,0])<10:
                    fig.savefig("im00000{}".format(b[n-1])+'.jpg', dpi=dpi, transparent=True) 
                    
                elif 10<=int(data[i-1,0])<100:
                    fig.savefig("im0000{}".format(b[n-1])+'.jpg', dpi=dpi, transparent=True)  
                    
                elif 100<=int(data[i-1,0])<1000:
                    fig.savefig("im000{}".format(b[n-1])+'.jpg', dpi=dpi, transparent=True)    
                    
                else:
                    fig.savefig("im00{}".format(b[n-1])+'.jpg', dpi=dpi, transparent=True)    
